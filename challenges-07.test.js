...
const forLoopTwoToThe = (arr) => {
  ...
  //number.push(Math.pow(2,element))
...
describe.only('Testing challenge 7', () => {
  test('It should return an array containing objects with name and total values', () => {
    expect(extractStatss(snorlaxStats.stats)).toStrictEqual([
      { name: 'speed', total: 35, },
      { name: 'special-defense', total: 112, },
      { name: 'special-attack', total: 74, },
    ]);
    expect(extractStatss(snorlaxStats.stats).length).toStrictEqual(3);
  });
});
